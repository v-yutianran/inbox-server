...
当前结论是维持 Worker 单副本，不创建多副本 ADR，也不修改 Sealos `replicas: 1`：

- D1 领取租约与 effect 幂等已具备独占/去重契约；文章仓库尚无跨副本写锁，浏览器登录态仍由单一持久会话持有，任务类型也没有排他的分片所有权
- active-passive 必须同时通过租约独占、effect 幂等、文章归档写锁和登录态隔离；任务分片还必须额外通过分片所有权门禁
- 当前合成模型只批准已验证包络：持续 10 分钟、0.2 task/s（720 task/h）、并发 4、浏览器任务占比 10%、浏览器任务 12 秒且代理无停机；这不是生产容量结论，14 天基线完成前不得外推更高上限

超过已验证包络或最老可执行任务年龄持续增长时，按以下顺序降级：

1. 保持单副本，停止人工同步、canary、重放和其它非必要增量操作，让 D1 队列保留任务而不是并发扩大副作用
2. 代理未就绪时继续停止领取；恢复后由现有租约和重试路径消化积压，不绕过限速、不清理 DLQ
3. 若已验证包络仍无法恢复积压，先完成 14 天容量证据，再重新运行多副本安全模型；只有全部门禁通过并批准独立 ADR 后才允许 active-passive 或分片
