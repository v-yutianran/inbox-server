...
### 8. 先用固定合成快照证明恢复编排，不读取生产 PVC

Worker、浏览器和 WARP 三类持久状态先以仓库内固定合成快照完成独立恢复演练。计划使用唯一运行标识、稳定 `planHash`、固定允许路径和 SHA-256；实际恢复只写唯一临时目录，重新读取三类 schema、稳定标识和记录数后清理。RTO 从临时恢复开始到三类启动门禁通过为止，目标 15 分钟；每日备份对应的 24 小时候选窗口可验证，但生产 RPO 在 `OPEN-004` 批准前始终标记为未批准。

替代方案是直接复制 Sealos PVC 或导入真实浏览器/WARP 状态。它能更接近生产，但会使用真实状态并产生生产数据复制边界，当前没有相应授权。固定合成快照不能证明平台快照 API 或真实进程重注册，只用于先消除恢复工具链、完整性校验和清理流程的不确定性。
...
- [合成状态不能证明真实 PVC 与进程恢复] → 证据明确限定为隔离候选验证，生产 RPO 与真实恢复保持阻塞并需单独授权。
...
7. 先完成三类固定合成快照恢复与候选 RPO 演练；取得真实数据与生产恢复授权后再验证平台快照和批准生产 RPO。
8. 完成容量/恢复演练后决定是否提出多副本 ADR；未通过则保持单副本并更新容量上限。
