...
- [x] 7.3 RED：为 Worker、浏览器状态和 WARP 固定合成快照定义严格 manifest、完整性失败、启动门禁、RTO/RPO、对账、脱敏与清理测试
- [x] 7.4 GREEN：实现 `rehearse-state-restore` 纯计划与文件系统执行器，dry-run 零 IO，实际执行必须确认同一 `planHash`
- [x] 7.5 REFACTOR：复用不可覆盖的 0600 证据写入器，完成 `TC-002` 隔离恢复并读回 RTO、候选 RPO、三类摘要、权限、零副作用与零残留
- [ ] 7.6 在 `OPEN-004` 决策并获真实平台备份恢复授权后，验证最终生产 RPO；不得用 `TC-002` 候选日窗口替代
- [x] 7.7 完成多副本租约、幂等、副作用、归档锁和登录态隔离测试，形成 active-passive、分片或维持单副本的决策数据
  - 纯模型覆盖租约独占、effect 幂等、文章归档写锁、登录态隔离与分片所有权；当前写锁/登录态/分片门禁未通过，结论为维持单副本
- [x] 7.8 若决定启用多副本或外部 APM，先使用 `docs-manager` 新建并批准独立 ADR；否则记录容量上限与降级策略
  - 未启用多副本或外部 APM；runbook 记录已验证合成包络 0.2 task/s（720 task/h）及三级降级顺序，因此无需新建 ADR
- [ ] 7.9 在旧 Compose 回滚窗口结束且获得删除授权后，另行制定 Python 运行时和旧部署资产的收敛计划
