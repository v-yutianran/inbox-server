...
### Requirement: 可验证的持久状态备份恢复
Worker、浏览器和 WARP 持久状态 SHALL 具备可验证的备份恢复流程；隔离演练 MUST 使用合成快照、唯一临时目录和稳定对账标识，且不得读取或改写生产状态。

#### Scenario: 隔离恢复三类持久状态
- **WHEN** 操作者使用同一计划先 dry-run，再恢复摘要匹配的 Worker、浏览器和 WARP 合成快照
- **THEN** dry-run 零写入，实际恢复在 15 分钟内完成，三类 schema、文件数、稳定标识和摘要对账一致，真实外部调用和临时残留均为 0

#### Scenario: 生产 RPO 尚未批准
- **WHEN** 隔离恢复完成但生产 RPO 仍处于未批准状态
- **THEN** 证据明确标记 RPO 未批准，只报告候选窗口验证结果且不得宣告生产恢复目标达成

#### Scenario: 快照完整性校验失败
- **WHEN** 任一快照存在路径越界、符号链接、重复路径、未允许文件或摘要不一致
- **THEN** 恢复在写入临时目录前失败，并产生不含敏感信息的失败证据
