...
- [ ] 4.3 实现任务幂等仓储与死信错误分类，验证业务状态持久化前不会 ack
- [x] 4.4 建立固定版本 Playwright Dockerfile 与 Xvfb 入口，验证 Node 为 PID 1 且 SIGTERM 可优雅退出
- [ ] 4.5 实现加密登录态适配器与 Sealos PVC 路径，验证日志不包含 Cookie、token 或完整 payload

## 5. 来源、目标与归档迁移

- [ ] 5.1 迁移一个无浏览器、无外部分发副作用的 collector，并完成新旧 shadow 契约对比
- [ ] 5.2 迁移一个 browser source，验证 headed Chromium、基线导入、来源键与去重键功能对等
- [ ] 5.3 逐项迁移剩余 HTTP/browser sources，并更新 parity checklist 与对应测试
- [ ] 5.4 逐项迁移 Cubox、Flomo、坚果云、通知与限速策略，并验证至少一次投递下不重复副作用
- [ ] 5.5 迁移 Defuddle/Eta 与 Git 文章归档，验证 Sealos PVC 重建后的仓库与去重状态

## 6. 数据迁移与双跑

- [ ] 6.1 实现 PostgreSQL 稳定快照导出、D1 幂等导入和按表计数/主键/不变量核对工具
- [ ] 6.2 使用本地或临时 D1 重复执行全量导入，生成不含敏感数据的核对报告
- [ ] 6.3 在禁用外部分发的 shadow 模式运行代表性任务，核对解析、凭据、幂等和错误分类
...
- [ ] 7.5 按浏览器资源阶梯完成冷启动、轻量页、真实页、一次交互和 60 秒稳定性手工验收
